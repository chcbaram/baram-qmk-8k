...
#ifdef USE_USB_HS
      USBD_CMPSIT_HIDKeyboardDesc(pdev, (uint32_t)pCmpstHSConfDesc, &CurrHSConfDescSz, (uint8_t)USBD_SPEED_HIGH);
#endif /* USE_USB_HS */
...
#if USBD_CMPSIT_ACTIVATE_HID == 1
/**
  * @brief  USBD_CMPSIT_HIDKeyboardDesc
  *         Configure and Append the HID Keyboard Descriptor
  * @param  pdev: device instance
                                      ...
                                      __IO uint32_t *Sze, uint8_t speed)
{
  ...
  __USBD_CMPSIT_SET_IF(pdev->tclasslist[pdev->classId].Ifs[0],
                       ...
                       0x01U, /* nInterfaceProtocol : 0=none, 1=keyboard, 2=mouse */
                       0U);   /* iInterface: Index of string descriptor */
  ...
  pHidKeyboardDesc->wItemLength         = HID_KEYBOARD_REPORT_DESC_SIZE;
...
static void  USBD_CMPSIT_CDCDesc(USBD_HandleTypeDef *pdev, uint32_t pConf, __IO uint32_t *Sze, uint8_t speed)
{
  ...
  *Sze                        += (uint32_t)sizeof(USBD_IadDescTypeDef);
  ...
  __USBD_CMPSIT_SET_IF(pdev->tclasslist[pdev->classId].Ifs[0],
                       0x0U, /* bAlternateSetting: Alternate setting */
                       0x1U, /* bNumEndpoints: One endpoint used */
