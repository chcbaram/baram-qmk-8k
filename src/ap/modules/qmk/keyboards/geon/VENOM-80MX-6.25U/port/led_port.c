...
void led_init_ports(void)
{
  ...
  if (led_config[LED_TYPE_SCROLL].mode != 1)
  {
    led_config[LED_TYPE_SCROLL].mode = 1;
    led_config[LED_TYPE_SCROLL].enable = true;
    led_config[LED_TYPE_SCROLL].hsv    = (HSV){HSV_GREEN};
...
void led_update_ports(led_t led_state)
{
  ...
  ws2812SetColor(1, led_color);


  led_color = WS2812_COLOR_OFF;
  if (led_config[LED_TYPE_SCROLL].enable && led_state.scroll_lock)
  {
    rgb_color = hsv_to_rgb(led_config[LED_TYPE_SCROLL].hsv);
    led_color = WS2812_COLOR(rgb_color.r, rgb_color.g, rgb_color.b);
  }
